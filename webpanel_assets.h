...
<head>
  ...
  <title>Minecraft Speed Proxy Panel</title>
...
<body>
  <div class="app">
    <header class="topbar">
      <div>
        <div class="brand">Minecraft Speed Proxy</div>
        <div class="subtle">Web panel</div>
      </div>
      <div class="toolbar">
        <select id="activeProxySelect" class="server-select" aria-label="Active server"></select>
        <span id="connectionState" class="status-pill status-warn">Disconnected</span>
    ...
    <section id="panelView" class="hidden">
      <nav class="tabs" aria-label="Tabs">
        ...
        <button type="button" class="tab" data-tab="servers">Servers</button>
        <button type="button" class="tab" data-tab="users">Users</button>
        <button type="button" class="tab" data-tab="lists">Lists</button>
        <button type="button" class="tab" data-tab="proxies">Proxies</button>
      ...
      <section id="overview" class="section tab-panel">
        <div class="metric-grid">
          <div class="metric"><div class="label">Online</div><div id="metricOnline" class="value">-</div></div>
          <div class="metric"><div class="label">Max</div><div id="metricMax" class="value">-</div></div>
          <div class="metric"><div class="label">Whitelist</div><div id="metricWhitelist" class="value">-</div></div>
          <div class="metric"><div class="label">Default Proxy</div><div id="metricProxy" class="value long-text">-</div></div>
          <div class="metric"><div class="label">Uptime</div><div id="metricUptime" class="value">-</div></div>
          <div class="metric"><div class="label">Started</div><div id="metricStart" class="value long-text">-</div></div>
        </div>

        <div class="grid-2">
          <div class="panel-box">
            <div class="toolbar">
              <button id="toggleWhitelistBtn" class="secondary" type="button">Toggle Whitelist</button>
              <button id="reloadMotdBtn" class="secondary" type="button">Reload MOTD</button>
            </div>
          ...
          <div class="panel-box">
            <form id="maxPlayerForm">
              <label for="maxPlayerInput">Max Players</label>
              <div class="form-row">
        ...
        <div class="grid-2">
          <div class="panel-box">
            <form id="serverForm" class="stack-form">
              ...
              <input id="serverName" type="text" placeholder="Name">
              <div class="form-row">
                <input id="serverLocalAddress" type="text" placeholder="0.0.0.0">
                <input id="serverLocalPort" type="number" min="1" max="65535" step="1" placeholder="Local port">
              </div>
              <div class="form-row">
                <input id="serverRemoteAddress" type="text" placeholder="Remote address">
                <input id="serverRemotePort" type="number" min="1" max="65535" step="1" placeholder="Remote port">
              </div>
              <div class="form-row">
                <input id="serverMaxPlayer" type="number" step="1" placeholder="-1">
                <input id="serverMotdPath" type="text" placeholder="MOTD path">
              </div>
        ...
        <div class="grid-2">
          <div class="panel-box">
            <div class="toolbar">
              ...
              <button id="whitelistToggleBtn" class="secondary" type="button">Toggle</button>
            </div>
            <form id="whiteAddForm" class="form-row">
              <input id="whiteAddInput" type="text" placeholder="Username">
          ...
          <div class="panel-box">
            ...
            <form id="blackAddForm" class="form-row">
              <input id="blackAddInput" type="text" placeholder="Username">
              <button class="primary" type="submit">Add</button>
            </form>
      ...
      <section id="proxies" class="section tab-panel hidden">
        <div class="grid-2">
          <div class="panel-box">
            <div class="label">Default Proxy</div>
            <div id="defaultProxyText" class="value long-text">-</div>
            <form id="userProxyForm" class="stack-form">
              <label for="proxyUsername">User Proxy</label>
              <input id="proxyUsername" type="text" placeholder="Username">
              <div class="form-row">
                <input id="proxyAddress" type="text" placeholder="Address">
                <input id="proxyPort" type="number" min="1" max="65535" step="1" placeholder="Port">
              </div>
              <button class="primary" type="submit">Save</button>
            </form>
          </div>
      ...
      <section id="motd" class="section tab-panel hidden">
        <div class="toolbar">
          <button id="motdPrettyBtn" class="secondary" type="button">Pretty</button>
          <button id="motdReloadBtn" class="secondary" type="button">Reload</button>
          <button id="motdSaveBtn" class="primary" type="button">Save</button>
        </div>
...
inline auto GetWebPanelJs() -> const std::string&
{
  ...
  function showLogin(message = "") {
    ...
    setConnectionState("Disconnected", "status-warn");
  ...
  function showPanel() {
    ...
    setConnectionState("Connected", "status-ok");
  }
  ...
  function renderOverview(status) {
    ...
    els.metricMax.textContent = maxPlayers < 0 ? "Unlimited" : String(maxPlayers);
    els.metricWhitelist.textContent = whitelistOn ? "On" : "Off";
    ...
    els.serverInfo.textContent = [
      `Server: ${status.proxy_name || status.proxy_id || "-"}`,
      `Listen: ${status.listen_endpoint || "-"}`,
      `Online: ${online}`,
      `Max players: ${maxPlayers < 0 ? "Unlimited" : maxPlayers}`,
      `Whitelist: ${whitelistOn ? "On" : "Off"}`,
      `Default proxy: ${status.default_proxy || "-"}`,
  ...
  function renderUsers(users) {
    ...
    if (!users.length) {
      els.usersTable.innerHTML = '<tr><td colspan="7" class="note">No online users</td></tr>';
      return;
    }
    els.usersTable.innerHTML = users.map((user) => `
      <tr>
        ...
        <td class="action-cell">
          <button class="danger" type="button" data-action="kick-user" data-username="${escapeAttr(user.username || "")}">Kick</button>
        </td>
      </tr>
    `).join("");
  }
  ...
  function renderList(container, values, action) {
    container.innerHTML = "";
    if (!values.length) {
      container.innerHTML = '<div class="note">Empty</div>';
      return;
    }
    container.innerHTML = values.map((value) => `
      <div class="list-item">
        <div class="long-text">${escapeHtml(value)}</div>
        <button class="danger" type="button" data-action="${action}" data-value="${escapeAttr(value)}">Remove</button>
      </div>
    `).join("");
  }
  ...
  function renderProxies(data) {
    ...
    if (!proxies.length) {
      els.proxyTable.innerHTML = '<tr><td colspan="3" class="note">No user proxies</td></tr>';
      return;
    }
    els.proxyTable.innerHTML = proxies.map((item) => `
      <tr>
        ...
        <td class="action-cell">
          <button class="danger" type="button" data-action="remove-user-proxy" data-username="${escapeAttr(item.username || "")}">Remove</button>
        </td>
      </tr>
    `).join("");
  }
  ...
  function renderServers(data) {
    ...
    if (!servers.length) {
      els.serversTable.innerHTML = '<tr><td colspan="5" class="note">No servers</td></tr>';
      return;
    }
    els.serversTable.innerHTML = servers.map((item) => `
      <tr>
        ...
        <td class="action-cell">
          <button class="secondary" type="button" data-action="select-proxy" data-proxy-id="${escapeAttr(item.id || "")}">Select</button>
          <button class="danger" type="button" data-action="remove-proxy-server" data-proxy-id="${escapeAttr(item.id || "")}">Remove</button>
        </td>
      </tr>
    `).join("");
  }
  ...
  async function refreshAll(silent = false) {
    ...
    state.loading = true;
    if (!silent) setMessage(els.panelStatus, "Loading...");
    try {
      ...
      if (!state.selectedProxyId) {
        ...
        els.serverInfo.textContent = "No server selected";
        renderUsers([]);
        renderProxies({ user_proxies: [] });
        renderLogs([]);
        setMessage(els.overviewMessage, "No server selected");
        setMessage(els.panelStatus, "Ready");
        setConnectionState("Connected", "status-ok");
        return;
      }
      ...
      setMessage(els.overviewMessage, `Updated ${new Date().toLocaleTimeString()}`);
      setMessage(els.panelStatus, "Ready");
      setConnectionState("Connected", "status-ok");
      if (status.whitelist_status) {
        els.toggleWhitelistBtn.textContent = "Disable Whitelist";
        els.whitelistToggleBtn.textContent = "Disable";
      } else {
        els.toggleWhitelistBtn.textContent = "Enable Whitelist";
        els.whitelistToggleBtn.textContent = "Enable";
      }
    } catch (error) {
  ...
  async function handleLogin(event) {
    ...
    if (!password) {
      setMessage(els.loginMessage, "Password required", "status-bad");
      return;
    }
  ...
  async function saveMotd() {
    ...
    setMessage(els.motdMessage, "Saved", "status-ok");
    await refreshAll(true);
  }
  ...
  document.addEventListener("click", async (event) => {
    ...
    try {
      if (action === "kick-user") {
        ...
        if (proxyId && confirm(`Remove ${proxyId}?`)) {
  ...
  els.serverForm.addEventListener("submit", async (event) => {
    ...
    if (!payload.local_port || !payload.remote_address || !payload.remote_port) {
      setMessage(els.panelStatus, "Missing server fields", "status-bad");
      return;
    }
    try {
      ...
      setMessage(els.panelStatus, "Server created", "status-ok");
    } catch (error) {
      setMessage(els.panelStatus, error.message, "status-bad");
    }
  });
  els.maxPlayerForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setMessage(els.panelStatus, "Max players updated", "status-ok");
    } catch (error) {
      setMessage(els.panelStatus, error.message, "status-bad");
    }
  });
  ...
  els.reloadMotdBtn.addEventListener("click", async () => {
    try {
      await reloadMotd();
      setMessage(els.panelStatus, "MOTD reloaded", "status-ok");
    } catch (error) {
      setMessage(els.panelStatus, error.message, "status-bad");
    }
  });
  els.motdReloadBtn.addEventListener("click", async () => {
    try {
      await reloadMotd();
      setMessage(els.motdMessage, "Reloaded", "status-ok");
    } catch (error) {
      setMessage(els.motdMessage, error.message, "status-bad");
    }
  });
  els.motdPrettyBtn.addEventListener("click", () => {
    try {
      ...
      setMessage(els.motdMessage, "Formatted", "status-ok");
    } catch (error) {
      setMessage(els.motdMessage, error.message, "status-bad");
    }
  });
  ...
  els.userProxyForm.addEventListener("submit", async (event) => {
    ...
    if (!username || !address || !port) {
      setMessage(els.panelStatus, "Missing proxy fields", "status-bad");
      return;
    }
    try {
      ...
      els.proxyPort.value = "";
      setMessage(els.panelStatus, "User proxy saved", "status-ok");
    } catch (error) {
      setMessage(els.panelStatus, error.message, "status-bad");
    }
  });
